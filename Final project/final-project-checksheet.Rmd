...
### Data Wrangling (4 out of 8 required)

*Description: Students need not use every function and method introduced in STAT 184, but clear demonstration of proficiency should include proper use of 4 out of the following 8 topics from class: (+) various data verbs for general data wrangling like filter, mutate, summarise, arrange, group_by, etc. (+) joins for multiple data tables. (+) pivot_wider & pivot_longer to stack/unstack variables (+) regular expressions (+) reduction and/or transformation functions like mean, sum, max, min, n(), rank, pmin, etc. (+) user-defined functions (+) loops and control flow (+) machine learning*
...
(C) .Rmd Line number(s) for a pivot_longer or pivot_wider operation (or equivalent):
...
### Data Visualization (2 of 5 required)
...
(E) Project is submitted as a self-contained GitHub Repo in addition to the nb.html Report and Checksheet. 
